...
if now.min % 10 == 0 then
    ...
    fileContent=""
    for i = 1, 5 do
        if string.len(fileContent) <100 then
            local config = assert(io.popen('curl http://www.meteofrance.com/mf3-rpc-portlet/rest/pluie/' .. CITY_CODE))
            fileContent = config:read('*all')
            config:close()
        end
    end    

    if string.len(fileContent) <100 then
        print("API Error:"..string.len(fileContent))
    else
        ...
        if when==0 then 
            print('Pas de pluie annoncée')
        end
        if rainLevel == 0 then -- données indisponibles
        ...
        elseif rainLevel == 1 then
            ...
            elseif rainLevel == 4 then
                ...
                commandArray[#commandArray + 1] = { ['SendEmail'] = 'Domoticz Anomalie#' .. os.date("%H:%M") .. ' -- Anomalie dans le script : script_time_PrevisionPluie.lua<br> L\'Api de meteoFrance renvoie un JSON inattendu. niveauPluie est en cas non traité (devrait être de 0 , 1 à 4)<br>Contenu du Json : -------------<br>' .. fileContent .. '#' .. EMAIL }
            end
            ...
            if My.Time_Difference(otherdevices_lastupdate["Alerte Pluie dans l\'heure"]) > 3 * 60 * 60 then
                if NOTIFICATION_VOCAL == true and now.hour > 8 and now.hour <= 20 then
                    My.Speak("alexa","Je vous dérange un instant. Une pluie " .. rainLevelText .. " est annoncée " .. whenText, "normal")
                end
